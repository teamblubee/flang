...
extern "C" __m128d __fvd_pow_fma3(__m128d, __m128d);
...
__m128d_2 __internal_ddfma (__m128d_2 x, __m128d_2 y, __m128d_2 z)
{
    __m128d e;
    __m128d_2 t, m;
    t.y = _mm_mul_pd(x.y, y.y);
    t.x = _mm_fmsub_pd(x.y, y.y, t.y);
    t.x = _mm_fmadd_pd(x.y, y.x, t.x);
    t.x = _mm_fmadd_pd(x.x, y.y, t.x);

    m.y = _mm_add_pd (z.y, t.y);
    e = _mm_sub_pd (z.y, m.y);
    m.x = _mm_add_pd (_mm_add_pd(_mm_add_pd (e, t.y), t.x), z.x);
...
__m128d_2 __internal_ddadd_yisdouble(__m128d_2 x, __m128d_2 y)
{
    __m128d_2 z;
    __m128d e;
    z.y = _mm_add_pd (x.y, y.y);
    e = _mm_sub_pd (x.y, z.y);
    z.x = _mm_add_pd(_mm_add_pd (e, y.y), x.x);
...
__m128d __attribute__ ((noinline)) __pgm_exp_d_vec128_slowpath(__m128i const i, __m128d const t, __m128d const bloga, __m128d z, __m128d prodx)
{
    ...
    __m128d lt_zero_mask = _mm_cmp_pd(bloga, ZERO, _CMP_LT_OS); // compute bloga < 0.0  
    __m128d slowpath_mask = _mm_cmp_pd(abs_lt, UPPERBOUND_1, _CMP_LT_OS); 

    __m128d a_plus_inf = _mm_add_pd(bloga, INF); // check if bloga is too big      
    ...
    __m128d z_fixed = _mm_fmadd_pd(z, prodx, z); // add only if not inf
...
__m128d __attribute__ ((noinline)) __pgm_pow_d_vec128_special_cases(__m128d const a, __m128d const b, __m128d t)
{
   ...
   __m128i const ELEVEN        = _mm_set1_epi64x(ELEVEN_D);
    ...
    __m128d b_ne_trunc = _mm_cmp_pd(b, _mm_round_pd(b, _MM_FROUND_TO_ZERO), _CMP_NEQ_UQ);
    ...
    __m128d absa_gt_one = _mm_cmp_pd(abs_a, ONE, _CMP_GT_OS); // evaluating (abs(a) > 1)
    ...
    t = _mm_blendv_pd(_mm_add_pd(a,b), t, (__m128d)aorb_nan_mask);
...
__m128d __fvd_pow_fma3(__m128d const a, __m128d const b)
{
    //////////////////////////////////////////////////// log constants
    ...
    thresh_mask = _mm_cmp_pd((__m128d)ihi, (__m128d)HI_THRESH,_CMP_GT_OS); 
    ...
    f = _mm_sub_pd(m, ONE_F);
    g = _mm_add_pd(m, ONE_F);
    g = _mm_div_pd(ONE_F, g); 

    u = _mm_mul_pd(_mm_mul_pd(TWO, f), g);
    ...
    __m128d c0 = _mm_fmadd_pd(LOG_POLY_1, v, LOG_POLY_0);
    __m128d c2 = _mm_fmadd_pd(LOG_POLY_3, v, LOG_POLY_2);
    __m128d c4 = _mm_fmadd_pd(LOG_POLY_5, v, LOG_POLY_4);
    __m128d v2 = _mm_mul_pd(v, v);
    __m128d v4 = _mm_mul_pd(v2, v2);

    c0 = _mm_fmadd_pd(c2, v2, c0);
    c4 = _mm_fmadd_pd(LOG_POLY_6, v2, c4);
    q = _mm_fmadd_pd(c4, v4, c0);
    q = _mm_mul_pd(q, v);

    tmp = _mm_mul_pd(TWO, _mm_sub_pd(f, u));
    tmp = _mm_fmadd_pd(neg(u), f, tmp);
    ulo = _mm_mul_pd(g, tmp);
    ...
    cc.y = _mm_mul_pd(uu.y, uu.y);
    cc.x = _mm_fmsub_pd(uu.y, uu.y, cc.y);
    cc.x = _mm_fmadd_pd(uu.y,
                           (__m128d)_mm_add_epi32((__m128i)uu.x, HALFIFIER), 
                           cc.x); // u ** 2

    tt.y = _mm_mul_pd(cc.y, uu.y);
    tt.x = _mm_fmsub_pd(cc.y, uu.y, tt.y);
    tt.x = _mm_fmadd_pd(cc.y, uu.x, tt.x);
    tt.x = _mm_fmadd_pd(cc.x, uu.y, tt.x); // u ** 3 un-normalized

    uu = __internal_ddfma(qq, tt, uu);

    // computing log a = log(m) + log(2)*expo
    f = __internal_fast_int2dbl (expo);
    q = _mm_fmadd_pd(f, LN2_HI, uu.y);
    tmp = _mm_fmadd_pd(neg(f), LN2_HI, q);
    tmp = _mm_sub_pd(tmp, uu.y);
    loga.y = q;
    loga.x = _mm_sub_pd(uu.x, tmp);
    loga.x = _mm_fmadd_pd(f, LN2_LO, loga.x);

    // finish log(a)
    // ******************************************************************************************
    
    // compute b * log(a)
    t_hi = _mm_mul_pd(loga.y, b);
    t_lo = _mm_fmsub_pd(loga.y, b, t_hi);
    t_lo = _mm_fmadd_pd(loga.x, b, t_lo);
    bloga = e = _mm_add_pd(t_hi, t_lo);
    prodx = _mm_add_pd(_mm_sub_pd(t_hi, e), t_lo);

    // ***************************************************************************************
    // computing exp(b * log(a))
    // calculating exponent; stored in the LO of each 64-bit block
    __m128i i = (__m128i) _mm_fmadd_pd(bloga, L2E, DBL2INT_CVT);

    // calculate mantissa
    __m128d t = _mm_sub_pd ((__m128d)i, DBL2INT_CVT);
    __m128d z = _mm_fmadd_pd (t, NEG_LN2_HI, bloga);
    z = _mm_fmadd_pd (t, NEG_LN2_LO, z);
    ...
    __m128d t4 = _mm_fmadd_pd(EXP_POLY_5, z, EXP_POLY_4);
    __m128d t6 = _mm_fmadd_pd(EXP_POLY_7, z, EXP_POLY_6);
    __m128d t8 = _mm_fmadd_pd(EXP_POLY_9, z, EXP_POLY_8);
    __m128d t10 = _mm_fmadd_pd(EXP_POLY_B, z, EXP_POLY_A);

    __m128d z2 = _mm_mul_pd(z, z);
    __m128d z4 = _mm_mul_pd(z2, z2);

    t4 = _mm_fmadd_pd(t6, z2, t4);
    t8 = _mm_fmadd_pd(t10, z2, t8);
    t4 = _mm_fmadd_pd(t8, z4, t4);
    
    t = _mm_fmadd_pd(t4, z, EXP_POLY_3);
    t = _mm_fmadd_pd(t, z, EXP_POLY_2);
    t = _mm_fmadd_pd(t, z, EXP_POLY_1);
    t = _mm_fmadd_pd(t, z, EXP_POLY_0); 
    ...
    z = _mm_fmadd_pd(z, prodx, z);     

    if (__builtin_expect(exp_slowmask, 0)) {
        z = __pgm_exp_d_vec128_slowpath(i, t, bloga, ztemp, prodx); 
    }
    ...
    if(__builtin_expect(specMask, 0)) {
        return __pgm_pow_d_vec128_special_cases(a, b, z);
    }
    return z;
}
